...
//checking if request cotains multipart data
            ...
            //f_item=(FileItem)itr.next();
                        ...
                        if (field_name.equals("file_photo"))//write the name of image here it is img_product
                        {
                            ...
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\Items\\";//path for storing image
                            Random r = new Random();//to generate random number
                            int r_num = r.nextInt(1111) + 999;//values between this are generated

                            photo = "Item" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                      ...
                      // if (field_name.equals("file_qrcode"))//write the name of image here it is img_product
                   //  {
                       //   String ext = file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
                          //setting path to store image
                       //   File proj_path = new File(config.getServletContext().getRealPath("/"));
                       //  String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\QR\\";//path for storing image
                       //   Random r = new Random();//to generate random number
                       //   int r_num = r.nextInt(1111) + 999;//values between this are generated

                        //     qr = "QR_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                            //creating a file object
                          //  savedFile = new File(file_path + qr);//fn contains name of file
                          //  try {
                                //writing the file object
                          //    f_item.write(savedFile);  //writing to server means the folder             
...
String InsQry = "insert into tbl_item(item_name,item_details,item_price,subcategory_id,item_image,item_rackno) values('" + value[2] + "','" + value[3] + "','" + value[4] + "','" + value[1] + "','" + photo + "','"+ value[5] +"')";
   //System.out.println(InsQry);
        ...
        <script type="text/javascript">
            ...
            setTimeout(function() {
                window.location.href = '../../Admin/Item.jsp'
